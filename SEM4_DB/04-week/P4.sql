...
-- Note: CHECK constraint removed for compatibility with MySQL
...
-- Insert 6 Doctors (Doctors 5 and 6 will have no appointments)
...
INSERT INTO DOCTOR VALUES (5, 'Dr. Wilson', 'Oncology');
INSERT INTO DOCTOR VALUES (6, 'Dr. Miller', 'General Practice');

-- Insert 8 Patients (Patients 107 and 108 will have no appointments)
...
INSERT INTO PATIENT VALUES (107, 'George King', 'Denver');
INSERT INTO PATIENT VALUES (108, 'Hannah Scott', 'Phoenix');
...
-- I used LEFT JOIN starting from PATIENT to keep the logic simple
...
SELECT D.name AS Doctor_With_No_Appointments
FROM DOCTOR D
LEFT JOIN APPOINTMENT A ON D.doctor_id = A.doctor_id
WHERE A.appointment_id IS NULL;

-- 4. Show patients who never had an appointment
SELECT P.name AS Patient_With_No_Appointments
FROM PATIENT P
LEFT JOIN APPOINTMENT A ON P.patient_id = A.patient_id
WHERE A.appointment_id IS NULL;

-- 5. Write a NATURAL JOIN version of Q1
-- Natural join matches columns with the same name (doctor_id, patient_id)
